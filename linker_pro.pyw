...
# --- НАЛАШТУВАННЯ ДЛЯ ОНОВЛЕНЬ ---
VERSION = "7.7"
# Замініть ці посилання на ваші, коли створите репозиторій:
VERSION_URL = "https://raw.githubusercontent.com/ВАШ_ЛОГІН/LinkerPro/main/version.txt"
FILE_URL = "https://raw.githubusercontent.com/ВАШ_ЛОГІН/LinkerPro/main/linker_pro.pyw"
...
class LinkerApp:
    def __init__(self, root):
        ...
        self.lang_data = {
            'ukr': {
                ...
                'update_err': "Не вдалося перевірити оновлення"
            },
            'eng': { 'zone': "📥\n\nDROP FILE HERE\n\nor CLICK", 'zone_drop': "🚀\n\nRELEASE", 'dedup': "Remove duplicates", 'ping': "Check Status", 'clear': "Clear", 'last_res': "STATS", 'found': "FOUND", 'unique': "VALID", 'errors': "ERRORS", 'copy_btn': "📋 COPY", 'copied': "✅ COPIED", 'update_found': "Update available!", 'update_no': "Up to date", 'update_err': "Update check failed" },
            'rus': { 'zone': "📥\n\nПЕРЕТЯНИТЕ ФАЙЛ СЮДА\n\nили НАЖМИТЕ", 'zone_drop': "🚀\n\nОТПУСТИТЕ", 'dedup': "Удалять дубликаты", 'ping': "Проверить Ping", 'clear': "Очистить", 'last_res': "СТАТИСТИКА", 'found': "НАЙДЕНО", 'unique': "ВАЛИДНЫХ", 'errors': "ОШИБКИ", 'copy_btn': "📋 КОПИРОВАТЬ", 'copied': "✅ СКОПИРОВАНО", 'update_found': "Доступно обновление!", 'update_no': "Последняя версия", 'update_err': "Ошибка обновления" }
        }
        
        self.curr_lang = 'ukr'
        self.dark_mode = False
        self.colors = {
            'light': {'bg': '#f0f2f5', 'fg': '#1a1d21', 'card': '#ffffff', 'zone': '#ffffff', 'accent': '#0061ff', 'stat_bg': '#ffffff', 'border': '#dce1e8'},
        ...
        # Фонова перевірка оновлень при запуску
    ...
    def check_updates(self, manual=False):
        try:
            ...
            if online_version > VERSION:
                if messagebox.askyesno("Update", f"{self.lang_data[self.curr_lang]['update_found']} (v{online_version})\nDownload now?"):
                    webbrowser.open(FILE_URL) # Відкриваємо пряме посилання на завантаження
            elif manual:
                messagebox.showinfo("Update", self.lang_data[self.curr_lang]['update_no'])
        except:
            if manual: messagebox.showwarning("Update", self.lang_data[self.curr_lang]['update_err'])
    ...
    def setup_ui(self):
        ...
        for l in ['ukr', 'eng', 'rus']:
            btn = tk.Button(self.lang_frame, text=l.upper(), font=("Segoe UI", 8, "bold"),
                            command=lambda lang=l: self.set_language(lang), relief="flat", padx=8, pady=4)
            btn.pack(side="left", padx=2)
            self.apply_hover(btn, True)
        ...
        self.apply_hover(self.theme_btn, True)

        # Кнопка оновлення
        self.upd_btn = tk.Button(self.top_bar, text="🔄", font=("Segoe UI", 11), command=lambda: self.check_updates(True), relief="flat", width=4)
        self.upd_btn.pack(side="right", padx=2)
        self.apply_hover(self.upd_btn, True)

        self.action_zone = tk.Label(self.root, text="", font=("Segoe UI Variable Text", 11, "bold"),
            bg=self.colors['light']['zone'], fg="#6c757d", relief="flat", highlightthickness=1, 
            highlightbackground=self.colors['light']['border'], cursor="hand2", height=7)
        ...
        self.check_dedup = tk.Checkbutton(self.opt_frame, text="", variable=self.dedup_var, bg=self.colors['light']['bg'], font=("Segoe UI", 9))
        self.check_dedup.pack(anchor="w")
        self.check_ping = tk.Checkbutton(self.opt_frame, text="", variable=self.ping_var, bg=self.colors['light']['bg'], font=("Segoe UI", 9))
        self.check_ping.pack(anchor="w")

        self.style = ttk.Style()
        self.style.theme_use('clam')
        self.style.configure("Modern.Horizontal.TProgressbar", troughcolor='#dce1e8', bordercolor='#dce1e8', background='#0061ff', thickness=8)
        ...
        self.res_title = tk.Label(self.root, text="", font=("Segoe UI", 9, "bold"), bg=self.colors['light']['bg'], fg="#8a94a6")
        self.res_title.pack(pady=5)
        self.stats_container = tk.Frame(self.root, bg=self.colors['light']['bg'])
        ...
        self.clear_btn = tk.Button(self.root, text="", command=self.clear_results, font=("Segoe UI", 8, "bold"), relief="flat", bg="#dee2e6", fg="#495057", padx=25, pady=5)
        self.clear_btn.pack(pady=25)
        self.apply_hover(self.clear_btn, False)
    ...
    def apply_hover(self, widget, is_top):
        def on_e(e): 
            ...
            widget.config(bg=self.colors[m]['border'], fg=self.colors[m]['fg'])
        def on_l(e):
            m = 'dark' if self.dark_mode else 'light'
            widget.config(bg=self.colors[m]['bg'] if is_top else ("#dee2e6" if not self.dark_mode else self.colors[m]['zone']), fg=self.colors[m]['fg'] if is_top else ("#495057" if not self.dark_mode else self.colors[m]['fg']))
        widget.bind("<Enter>", on_e); widget.bind("<Leave>", on_l)

    def set_language(self, lang):
        self.curr_lang = lang
        d = self.lang_data[lang]
    ...
    def toggle_theme(self, save=True):
        ...
        m, c = ('dark', self.colors['dark']) if self.dark_mode else ('light', self.colors['light'])
        ...
        for ck in [self.check_dedup, self.check_ping]: ck.config(bg=c['bg'], fg=c['fg'], selectcolor=c['bg'] if not self.dark_mode else "#2d3339")
        self.opt_frame.config(bg=c['bg']); self.stats_container.config(bg=c['bg']); self.res_title.config(bg=c['bg'])
        self.clear_btn.config(bg=c['zone'] if self.dark_mode else "#dee2e6", fg=c['fg'] if self.dark_mode else "#495057")
